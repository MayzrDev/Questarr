...
describe('GET /api/indexers/search - Aggregated multi-indexer search', () => {
  ...
  const mockTorznabXmlResponse = (items: Array<{ title: string; link: string; seeders?: number; size?: number }>) => {
  ...
  it('should aggregate results from multiple enabled indexers', async () => {
    ...
    const response1 = {
      ...
      status: 200,
      statusText: 'OK',
      text: async () => mockTorznabXmlResponse([
  ...
  it('should return items and errors when some indexers fail', async () => {
    ...
    const successResponse = {
      ...
      statusText: 'OK',
      text: async () => mockTorznabXmlResponse([
        { title: 'Game From Working Indexer', link: 'http://example.com/game-working', seeders: 25 },
      ]),
    };
  ...
  it('should throw error when no enabled indexers are available', async () => {
    ...
    await expect(
      torznabClient.searchMultipleIndexers([], searchParams)
    ).rejects.toThrow('No enabled indexers available');
  });
